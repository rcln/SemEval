...
public class Analyzer {
  ...
  private static void printDepTree( int depth, TreeDepnode tr ) {
    ...
    if(tr.getInfo().isHead()) {
    	System.err.print(tr.getInfo().getLabel()+"(");
    	for( int i = 0; i < nch; i++ ) {
            child = tr.nthChildRef( i );
            if(child.getInfo().isHead()) {
            	System.err.print(child.getInfo().getLinkRef().getInfo().getLabel());
            } else {
            	System.err.print(child.getInfo().getWord().getLemma());
            }
            System.err.print(",");
        }
    	System.err.println(")");
    ...
    /*if(nch==0) System.out.print(tr.getInfo().getWord().getLemma());
    else {
    	for( int i = 0; i < nch; i++ ) {
            child = tr.nthChildRef( i );
            printDepTree(depth+1, child);
