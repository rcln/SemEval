...
public class DepPair {
	...
	public double getDepScore(){
		...
		for(Dependency k : alMapD1.keySet()){
			if(SpanishSTSComparer.VERBOSE) System.err.println("S1 Alignment: "+k.toString()+" <-> "+alMapD1.get(k).getDependency().toString()+" score: "+alMapD1.get(k).getScoreValue());
			sum1+=alMapD1.get(k).getScoreValue();
		}
		double score_1=sum1/N1;
		if(SpanishSTSComparer.VERBOSE) System.err.println("S1 Score: "+score_1+"\t(N:"+N1+")");
		
		for(Dependency u : alMapD2.keySet()){
			if(SpanishSTSComparer.VERBOSE) System.err.println("S2 Alignment: "+u.toString()+" <-> "+alMapD2.get(u).getDependency().toString()+" score: "+alMapD2.get(u).getScoreValue());
			sum2+=alMapD2.get(u).getScoreValue();
		}
		double score_2=sum2/N2;
		if(SpanishSTSComparer.VERBOSE) System.err.println("S2 Score: "+score_2+"\t(N:"+N2+")");
